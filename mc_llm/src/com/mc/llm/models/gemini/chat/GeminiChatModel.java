...
public class GeminiChatModel implements BaseModel {
	...
	private GeminiChatModel(String url) {
		...
		if (API_KEY == null) throw new LLMException(ErrorCode.BAD_REQUEST, "GEMINI_API_KEY 를 확인해주세요");
	}
		...
		try (HttpClient client = HttpClient.newHttpClient()) {
			...
			if (response.statusCode() < 200 || response.statusCode() > 299) {
				...
				if (response.statusCode() == 404) {
					throw new LLMException(ErrorCode.BAD_REQUEST, "404 api url error");
				}
				
				JsonObject jsonObject = JsonProvider.gson().fromJson(response.body(), JsonObject.class);
				GeminiErrorMessage errorMessage = JsonProvider.gson().fromJson(jsonObject.get("error"), GeminiErrorMessage.class);
