...
public class GeminiRequest<T extends Format> implements BaseRequest {
	...
	public String messageToJson() {
		ResponseSchema<T> schema = new ResponseSchema<T>(new Items<>(format));
		GenerationConfig<T> config = new GenerationConfig<>(schema);
		RequestDocument<T> doc = new RequestDocument<T>(List.of(new Content(List.of(new Part(content)))), config);
